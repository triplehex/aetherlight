...
export class Player extends ScriptModule {
    ...
    update(world: ScriptWorld, entityId: string) {
        ...
        let cameraIds = world.taggedEntities('Camera');
        cameraIds.forEach(id => {
            // Only our actual local camera will have the camera component
            var cameraRotation: Quat;
            try {
                cameraRotation = new Quat(world.getCamera(id).rotation);
            } catch (e) {
                return;
            };
        ...
        if (Math.abs(controls.move_direction.x) > 0. || Math.abs(controls.move_direction.y) > 0.) {
            ...
            if (controls.sprint) {
                if (isOnGround) world.playAnimation(entityId, "fastrun", 0.2);
            } else {
                if (isOnGround) world.playAnimation(entityId, "slowrun", 0.2);
            }
        } else {
